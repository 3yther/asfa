"""Multi-source food search tests — USDA → Open Food Facts fallback chain.
...
real USDA/OFF calls are made. Verifies: USDA hits win (no fallback), USDA misses
fall back to OFF with source='open_food_facts', both-empty is cached, and a
transient outage on either side is NOT cached.
"""
import os
import sys
import tempfile
...
def _install(usda_by_q=None, off_by_q=None, usda_error=False, off_error=False):
    """Point nutrition.requests.get at a stub and reset the call counters +
    search cache so each test starts clean."""
    usda_by_q = usda_by_q or {}
    off_by_q = off_by_q or {}
    CALLS["usda"] = CALLS["off"] = 0
    ...
    def fake_get(url, *args, **kwargs):
        ...
        if "api.nal.usda.gov" in url:
            ...
            return _FakeResp({"foods": usda_by_q.get(q, [])})
        if "search.pl" in url:
            CALLS["off"] += 1
            if off_error:
...
def test_1_usda_hit_no_fallback():
    ...
    print("  1. 'honey' -> USDA hit, source=usda, no OFF call  OK")
...
def test_2_fallback_to_off():
    ...
    assert CALLS["usda"] == 1 and CALLS["off"] == 1, CALLS
    print("  2. 'amlu berry' -> USDA empty, OFF fallback, source=open_food_facts  OK")
...
def test_4_both_empty_cached():
    ...
    assert res == [], res
    assert CALLS["usda"] == 1 and CALLS["off"] == 1, CALLS
    # Second call must be served from cache (both sources were reached).
    res2 = nutrition.search_foods("zzzznope")
    assert res2 == [], res2
    assert CALLS["usda"] == 1 and CALLS["off"] == 1, ("empty should be cached", CALLS)
    print("  4. both empty -> [] and cached (no re-query)  OK")
...
def main():
    tests = [test_1_usda_hit_no_fallback, test_2_fallback_to_off,
             test_3_off_kj_fallback, test_4_both_empty_cached,
             test_5_transient_not_cached, test_6_endpoint_source_surfaced]
